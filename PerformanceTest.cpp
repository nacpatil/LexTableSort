...
void PerformanceTest::runPerformanceOnColumns (std::string algoType, std::string dataType, size_t numRows, size_t numCols) {
    // Open file to write results
    std::ofstream resultFile("performance_results_" + algoType + "_" + dataType +"_coltest" + ".csv");
    if (!resultFile) {
        std::cerr << "Error opening file for writing results!" << std::endl;
        return;
    }
    ...
    // Run performance tests for different row sizes from 1 million to 20 million with a 2 million gap
    for (size_t curCols = 1; curCols < numCols; curCols+=5) {
        ...
        // Write results to the file in columns (NumRows, SortTime)
        resultFile << curCols << "," << sort_duration.count() << "\n";
    }
...
#pragma omp parallel for
    for (ptrdiff_t i = 0; i < static_cast<ptrdiff_t>(size); ++i) {
        ...
        values[index] = (thisNumber + index - 1) % 9000;
    }
    return values;
}
